...
public class PointShapeInfo extends ShapeDebugFrame.ShapeInfo {
    private final rescuecore2.misc.geometry.Point2D point;
    ...
    public PointShapeInfo(rescuecore2.misc.geometry.Point2D point, String name, Color color,
                          final PointShape shape, final int size) {
        super(point, name);
        this.point = point;
        this.color = color;
    ...
    public Shape paint(final Graphics2D g, final ScreenTransform transform) {
        ...
        final int x = transform.xToScreen(point.getX());
        final int y = transform.yToScreen(point.getY());
